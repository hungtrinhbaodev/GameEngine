...
namespace Vulkan {
	...
	namespace Utils {
		...
		inline void vk_check_result(VkResult result, const char * success_msg = "Vulkan action success!", const char* exception_msg = "Vulkan action fail!") {
			switch (result) {
				case VK_SUCCESS: {
        ...
        inline Queue_Family_Indices query_suitable_queue_family_indices(VkPhysicalDevice physical_device, VkSurfaceKHR surface) {
        ...
        inline std::vector<const char*> query_physical_device_support_required_extensions(
            ...
            vkEnumerateDeviceExtensionProperties(physical_device, nullptr, &number_extension, extension_properties.data());
            std::vector<const char*> extension_names;
        ...
        inline bool is_suitable_physical_device(VkPhysicalDevice physical_device, VkSurfaceKHR surface) {
            ...
            bool is_device_support_require_extension = list_extensions.size() >= Const::REQUIRED_PHYSICAL_DEVICE_EXTENSIONS.size();

            // query capabilities of swap chain info in device
            bool swap_chain_adequate = false;
            if (is_device_support_require_extension) {
                Swapchain_Support_Detail details = query_swapchain_support_detail(
                    physical_device,
                    surface
                );
        ...
        inline std::vector<const char*> query_physical_device_layers_enabled(VkPhysicalDevice physical_device) {
            ...
            for (auto& layer_enable : layer_enables) {
                if (Const::IS_ENABLE_VALIDATION_LAYERS && strcmp(layer_enable.layerName, Const::VALIDATION_LAYER_NAME)) {
