...
namespace Vulkan {
	...
	namespace Init {
		...
		void _init_queues() {
			...
			Log::log_info("Get device graphic queue at index", indices.graphic_family.value(), graphics_queue,
						  "success!");

			vkGetDeviceQueue(device, indices.present_family.value(), 0, &present_queue);
			Log::log_info("Get device present queue at index", indices.present_family.value(), present_queue,
						  "success!");
		}
