...
namespace Vulkan {
    ...
    void Buffer::copy_buffer(Buffer src_buffer, Buffer dst_buffer, uint32_t src_offset, uint32_t dst_offset, uint32_t size) {
        ...
        if (src_buffer.is_host_visible_buffer() && dst_buffer.is_host_visible_buffer()) {
            ...
            _global_thread_pool->enqueue([](Buffer& src, Buffer& dst, VkBufferCopy& region) {
                ...
                API::on_fence_success(fence, [](VkCommandBuffer command_buffer, std::thread::id thread_id, VkFence fence) {
                    API::release_command_buffer(command_buffer, thread_id);
                    API::release_fence(fence);
                    }, command_buffer, thread_id, fence).get();
                }, src_buffer, dst_buffer, region).get();
        }
    }
