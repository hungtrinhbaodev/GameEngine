...
namespace Vulkan {
	...
	Vertex_Input_Builder& Vertex_Input_Builder::add_binding_description(uint32_t binding, uint32_t stride, VkVertexInputRate input_rate) {
	...
	Vertex_Input_Builder& Vertex_Input_Builder::add_attribute_description(uint32_t binding, VkFormat format, uint32_t offset) {
		if (binding >= attribute_descriptions_by_bindings.size()) {
	...
	Vertex_Input_Builder& Vertex_Input_Builder::add_mat4_attribute_description(uint32_t binding, uint32_t offset) {
		...
		for (int i = 0; i < 4; i++) {
			attribute_descriptions_by_bindings[binding].push_back({
				...
				offset + static_cast<uint32_t>(sizeof(float)) * 4 * i
			});
		}
		return *this;
	}
